...
test("bridge version check passes local MCP, Pod, and README synchronization", () => {
  ...
  assert.equal(result.mcpVersion, "0.1.12");
  assert.equal(result.podVersion, "0.1.12");
  assert.deepEqual(result.readmeTags, ["0.1.12"]);
});

test("latest bridge tag parser ignores annotated peeled refs and sorts semver", () => {
  const output = [
    "sha refs/tags/0.1.9",
    "sha refs/tags/0.1.12",
    "sha refs/tags/0.1.12^{}",
    "sha refs/tags/0.1.11",
  ].join("\n");
  assert.equal(parseLatestStableTag(output), "0.1.12");
});

test("README parser reads the documented bridge tag", () => {
  assert.deepEqual(
    parseREADMEBridgeTags("pod 'LookDebugBridge', :git => 'https://github.com/Immmmmmortal1/LookDebugBridgeService.git', :tag => '0.1.12'"),
    ["0.1.12"]
  );
});
